  ...
  for(let rep=0; rep<conf.repetitions; rep++){
    ...
    for(let r=0; r<conf.positions.length; r++){
      ...
      for(let c=0; c<conf.positions[r].length; c++){
        ...
        if(r == 0 && c == 0)
          point = initial;
        html = html + `<span style="position:absolute; top:${point.y}mm; left:${point.x}mm; font-size:${conf.fontSize}px">#token${tx.toString()}-${c.toString()}</span>`
      }
    }
  }
...
function replaceTokens(extinguishers, html) {
  values = [];
  extinguishers.forEach((extinguisher,i) => {
    let v = _getValues(extinguisher);
    v.forEach(row => {
      values.push(row)
    });
  });
...
function _getValues(extinguisher) {
...
module.exports = {
  ...
  inputs: {
    ids: {
      type: 'ref',
      description: 'Extinguishers IDs to print',
      required: true
    },
  ...
  exits: {
    ...
    error: {
      description: 'Algo no funcionó como se esperaba',
    },
  ...
  fn: async function (inputs, exits) {
    Extinguisher.find({'id': inputs.ids}).populate('type')
      .then(exts => {
        let config = exts[0].category == CATEGORY_TYPE.VEHICULAR ? v_conf : d_conf;
        let html = buildHtml(config);
        html = replaceTokens(exts, html);
